...
do
  ...
  then
    # insert into teams
    INSERT_WINNER_TEAM_RESPONSE=$($PSQL "INSERT INTO teams(name) VALUES('$WINNER')")
    if [[ $INSERT_WINNER_TEAM_RESPONSE = 'INSERT 0 1' ]]
    then
  ...
  then
    # insert into teams
    INSERT_OPPONENT_TEAM_RESPONSE=$($PSQL "INSERT INTO teams(name) VALUES('$OPPONENT')")
    if [[ $INSERT_OPPONENT_TEAM_RESPONSE = 'INSERT 0 1' ]]
    then
  ...
  then
    echo "added to games: $YEAR, '$ROUND', $WINNER_ID, $OPPONENT_ID, $WINNER_GOALS, $OPPONENT_GOALS"
  fi
  fi
done